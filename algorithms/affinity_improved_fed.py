...
def shared_light_newcore(light_task, partitioned_ab_org, available_cores):
    ...
    if len(partitioned_a[0]) == 0 and light_task[1].utilizationA > 0 and len(partitioned_b[0]) != 0:
        new_processor = []
        new_task = []
    ...
    if len(partitioned_b[0]) == 0 and light_task[1].utilizationB > 0 and len(partitioned_a[0]) != 0:
        new_processor = []
        new_task = []
        new_task.append(light_task)
        new_task.append(light_task[0].utilization * light_task[0].period)
